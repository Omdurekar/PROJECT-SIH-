 ...
 *
 * Core Features:
 * 1. Immediate DOM view transition on Register submit:
 *    - Validates required fields (Username, Email, Password)
 *    - Hides registration form (#registerForm)
 *    - Shows OTP verification view (#otpForm)
 *    - Auto-focuses the first OTP digit input box
 *    - Starts 60-second disabled countdown on Resend button
 *    - Starts 2-minute OTP expiry countdown
 * 2. Asynchronous background registration dispatch
 * 3. 6-digit numeric input with auto-focus movement, backspace & paste support
 * 4. Verify OTP (POST /api/v1/auth/verify-otp) with transition to login
 * 5. Resend OTP (POST /api/v1/auth/resend-otp) with timer resets
 * 6. Login (POST /api/v1/auth/login) with 403 Forbidden unverified handling
 */

(function () {
  'use strict';

  // Prevent any legacy script crashes by ensuring safety dummies
  ...
  // --- 3. 6-DIGIT OTP BOX INTERACTION ENGINE ---
  ...
  function showOtpVerificationView(email) {
    ...
    // 1. Hide registration & login containers
    if (loginForm) {
      loginForm.classList.remove('active');
      loginForm.classList.add('hidden');
      loginForm.style.display = 'none';
    ...
    // 2. Show OTP verification container
    if (otpForm) {
    ...
    // 4. Auto-focus first digit & reset inputs
    clearOtpInputs();
    focusFirstOtpDigit();

    // 5. Start timers (60s disabled countdown & 2-minute expiry countdown)
    startBothTimers();
  }

  // --- 5. IMMEDIATE TRANSITION & REGISTRATION HANDLER ---
  function handleRegisterTransition(event) {
    ...
    // Validate that basic required fields (Username, Email, Password) are populated
    ...
    // DYNAMIC TRANSITION: Immediately hide register fields & show OTP view
    showOtpVerificationView(email);

    // Asynchronously dispatch backend register request
  ...
  // --- 8. USER LOGIN & 403 FORBIDDEN HANDLER ---
  ...
  // --- 9. DOM INITIALIZATION & ATTACHMENT ---
  function initAuthWiring() {
    setupOtpDigitInputs();

    // 1. Register Form Submit & Create Account Button Click Interception
    const registerForm = document.getElementById('registerForm');
    if (registerForm) {
    ...
    // 2. OTP Form Submit Interception
    const otpForm = document.getElementById('otpForm');
    if (otpForm) {
      otpForm.addEventListener('submit', handleVerifyOtp, { capture: true });
    }

    // 3. Login Form Submit Interception
    const loginForm = document.getElementById('loginForm');
    if (loginForm) {
    ...
    // 6. Global Function Expositions for inline HTML attributes
